...
ipcMain.handle('DESKTOP_CAPTURER_GET_SOURCES', async (_event, opts: Electron.SourcesOptions) => {
  ...
  return Promise.all(
    sources.map(async (s) => ({
      ...
      display_id: (s as any).display_id as string | undefined,
    }))
  );
});

ipcMain.handle('DB_GET_META', (_event, key: string) => {
...
ipcMain.handle('DB_SET_META', (_event, key: string, value: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_SAVE_MESSAGE', (_event, msg: DbMessageRow) => {
  const database = getDb();
  if (!database) {
  ...
  database
    .prepare(
      `INSERT INTO messages (id, room_id, peer_id, direction, created_at, status, ttl_ms, nonce_b64, payload_b64)
...
ipcMain.handle('DB_GET_MESSAGES', (_event, roomId: string, limit = 200) => {
  const database = getDb();
  if (!database) {
  ...
  const rows = database
    ...
    .all(roomId, limit) as DbMessageRow[];
  return rows;
});

// ==================== Window Controls ====================
ipcMain.handle('WINDOW_CONTROL', (_event, action: string) => {
  ...
  switch (action) {
    ...
    case 'focus': mainWindow.focus(); break;
  }
});
...
ipcMain.handle('WINDOW_SET_FULLSCREEN', (_event, flag: boolean) => mainWindow?.setFullScreen(flag));
ipcMain.handle('WINDOW_SET_ALWAYS_ON_TOP', (_event, flag: boolean) => mainWindow?.setAlwaysOnTop(flag));

// ==================== Contacts ====================
ipcMain.handle('DB_SAVE_CONTACT', (_event, contact: ContactRow) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_GET_CONTACT', (_event, id: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_GET_ALL_CONTACTS', () => {
  ...
  return rows.map((r: any) => ({ ...r, verified: !!r.verified, blocked: !!r.blocked }));
});

ipcMain.handle('DB_DELETE_CONTACT', (_event, id: string) => {
  const database = getDb();
  if (!database) {
...
// ==================== Conversations ====================
ipcMain.handle('DB_SAVE_CONVERSATION', (_event, conv: ConversationRow) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_GET_CONVERSATION', (_event, id: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_DELETE_CONVERSATION', (_event, id: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_DELETE_META', (_event, key: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_DELETE_MESSAGE', (_event, id: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_DELETE_MESSAGES_BY_ROOM', (_event, roomId: string) => {
  const database = getDb();
  if (!database) {
...
ipcMain.handle('DB_VACUUM', () => {
  const database = getDb();
  if (!database) return; // No-op for memory store
  database.prepare('VACUUM').run();
});

// ==================== Notifications ====================
ipcMain.handle('NOTIFICATION_SHOW', (_event, options: { title: string; body: string; silent?: boolean }) => {
  if (Notification.isSupported()) {
    new Notification({ ...options, icon: undefined }).show();
  }
});

ipcMain.handle('NOTIFICATION_SET_BADGE', (_event, count: number) => {
...
// ==================== File System ====================
ipcMain.handle('DIALOG_OPEN_FILE', async (_event, options: Electron.OpenDialogOptions) => {
  const result = await dialog.showOpenDialog(mainWindow!, options);
  return result.canceled ? null : result.filePaths;
});

ipcMain.handle('DIALOG_SAVE_FILE', async (_event, options: Electron.SaveDialogOptions) => {
...
ipcMain.handle('FS_WRITE_FILE', async (_event, filePath: string, data: Uint8Array) => {
...
// ==================== Clipboard ====================
ipcMain.handle('CLIPBOARD_WRITE_TEXT', (_event, text: string) => {
  clipboard.writeText(text);
});

ipcMain.handle('CLIPBOARD_READ_TEXT', () => clipboard.readText());

ipcMain.handle('CLIPBOARD_WRITE_IMAGE', (_event, dataUrl: string) => {
...
ipcMain.handle('SHELL_SHOW_ITEM_IN_FOLDER', (_event, fullPath: string) => {
...
ipcMain.handle('SECURE_STORAGE_SET', (_event, key: string, value: string) => {
  secureStore.set(key, value);
});

ipcMain.handle('SECURE_STORAGE_GET', (_event, key: string) => {
  return secureStore.get(key) ?? null;
});

ipcMain.handle('SECURE_STORAGE_DELETE', (_event, key: string) => {
  secureStore.delete(key);
});

ipcMain.handle('SECURE_STORAGE_HAS', (_event, key: string) => {
