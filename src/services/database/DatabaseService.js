...
class DatabaseService {
    ...
    getUserIdentity() {
        ...
        return this.rowToObject(cols, row);
    }
    // ==================== Contacts ====================
    saveContact(contact) {
        if (!this.db)
    ...
    getAllContacts() {
        ...
        if (!result.length)
            return [];
        return result[0].values.map((row) => this.rowToContact(result[0].columns, row));
    }
    deleteContact(id) {
        if (!this.db)
            return;
    ...
    `);
        if (!result.length)
            return [];
        return result[0].values.map((row) => this.rowToConversation(result[0].columns, row));
    }
    ...
    getMessages(conversationId, limit = 100, beforeId) {
        if (!this.db)
            return [];
        let query = `
    ...
    `;
        ...
        if (!result.length)
            return [];
        return result[0].values.map((row) => this.rowToMessage(result[0].columns, row)).reverse();
    }
    ...
    deleteMessage(id, hardDelete = false) {
        if (!this.db)
            return;
        if (hardDelete) {
    ...
    rowToConversation(columns, values) {
        const obj = this.rowToObject(columns, values);
        return {
            ...obj,
